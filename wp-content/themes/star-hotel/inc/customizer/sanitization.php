 ...
 */
function starhotel_sanitize_checkbox( $checked ) {
	// Boolean check.
...
function starhotel_switch_sanitization( $input ) {
    if ( true === $input ) {
        return 1;
    } else {
        return 0;
...
function starhotel_sanitize_choices( $input, $setting ) {
    global $wp_customize; 
...
function starhotel_sanitize_float( $input ) {
 ...
 */
function starhotel_sanitize_hex_color( $hex_color, $setting ) {
 ...
 */
function starhotel_sanitize_html( $html ) {
 ...
 */
function starhotel_sanitize_image( $image, $setting ) {
 ...
 */
function starhotel_sanitize_nohtml( $nohtml ) {
 ...
 */
function starhotel_sanitize_number_absint( $number, $setting ) {
 ...
 *                    the setting default.
 */
function starhotel_sanitize_range_value( $number, $setting ) {
 ...
 */
function starhotel_sanitize_select( $input, $setting ) {
 ...
 */
function starhotel_sanitize_url( $url ) {
	return esc_url_raw( $url );
}


/* Sanitization Text*/
function starhotel_sanitize_text( $text ) {
	return wp_filter_post_kses( $text );
}

/* Sanitization Integer*/
function starhotel_sanitize_integer( $input ) {
    if( is_numeric( $input ) ) {
        return intval( $input );
    }
}
